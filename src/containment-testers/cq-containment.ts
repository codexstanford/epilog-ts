...
function freezeCQ(cq : CQ) : CQ {
    ...
    let freezingMap : Map<string, Term> = new Map();
...
class CQContainmentTester implements ContainmentTester {
    ...
    containedWithin(q1: Query, q2: Query): boolean {
        ...
        let frozenDB : Dataset = new Dataset(frozenBody);
        ...
        let epilogJSResults : EpilogJSToTS.EpilogJSAtom[] = compfinds(epilogJSQ2Head, epilogJSQ2Body, definemorefacts([],epilogJSDataset), []);
            
        let frozenHeadStr : string = frozenCQ.rule.head.toString();
        ...
        for (let epilogJSAtom of epilogJSResults) {
            let result : Atom = EpilogJSToTS.parseAtom(epilogJSAtom);

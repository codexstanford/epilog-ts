...
import { CompoundTerm, Symbol, Term, Variable, applySubtoTerm } from "../../epilog-ts-core/classes/Term.js";


// Recursive helper function to unify Terms
function tryGetMGU_helper(term1 : Term, term2: Term, currSub: Substitution) : Substitution | false {
    ...
    if (term1 instanceof CompoundTerm && 
        ...
        for (let j = 0; j < term1.args.length; j++) {
            ...
            let tryUnifyResult : Substitution | false = tryGetMGU_helper(currArg1, currArg2, currSub);
...
function tryGetMGU_literal(lit1 : Literal, lit2 : Literal) : Substitution | false {
    ...
    for (let i = 0; i < lit1.atom.args.length; i++) {
        ...
        let currLit1Arg : Term = applySubtoTerm(mgu, lit1.atom.args[i]);
        let currLit2Arg : Term = applySubtoTerm(mgu, lit2.atom.args[i]);
        
        let argUnificationResult : Substitution | false = tryGetMGU_helper(currLit1Arg, currLit2Arg, mgu);
...
function tryGetMGU(expr1 : Literal | Term, expr2: Literal | Term) : Substitution | false {
    // expr1 and expr2 must not have any Variables in common
    let expr1Vars : Set<string> = expr1.getVars();
    let expr2Vars : Set<string> = expr2.getVars();

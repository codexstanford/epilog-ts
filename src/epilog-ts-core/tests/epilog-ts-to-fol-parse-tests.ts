...
import { standardizeRuleVars, standardizeRuleHead } from "../utils/standardize.js";

// Unit tests for epilog-ts-core/parsing/epilog-ts-to-fol
function runTests() : void {
    runRuleListTests();
}

function runRuleListTests() : void {
    printTestingMessage_Start("Rule List - Single Rule")

    runTest("Rule List-empty-success", () => {
        let f : Formula = EpilogTSToFOL.parseRuleList([]);
    ...
    runTest("Rule List-single rule, no body no args no-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p()'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, no args-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q()'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head var-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 2 head var-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, Y) :- q(Y)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 3 head var, two identical-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, Y, X) :- q(Y, X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head constant-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c) :- q(c)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head compound term-1-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X)) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head compound term-2-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X, Y)) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head compound term, 1 head var-1-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X, Y), X) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 1 head compound term, 1 head var-2-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, f(X, Y)) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 3 head args of different types-1-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X), X, c) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 3 head args of different types-2-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, f(X), c) :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, 3 head args of different types, no body-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X), X, c, X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, no head args, 1 body var-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q(X)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, no head args, multiple body var-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q(f(X), X, c, X) & r(Y)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, multiple head args, multiple body var-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, Y, X) :- q(f(Z), Z, c, Z) & r(W)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-single rule, multiple head args, multiple body var, multiple same-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X, Y, f(X), S, S, T) :- q(f(Z), Z, c, Z) & r(W) & r(S) & t(T)'));
        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1]);
    ...
    runTest("Rule List-two rules, different head predicates-1-failure", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p()'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('q()'));

        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1, rule2]);
    ...
    runTest("Rule List-two rules, different head predicates-2-failure", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q()'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('q() :- p()'));
    ...
    runTest("Rule List-two rules, no head args, one with no body-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q()'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p()'));
    ...
    runTest("Rule List-two rules, no head args-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p() :- q()'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p() :- r()'));
    ...
    runTest("Rule List-two rules, 1 head var, same in each-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(X) :- r()'));
    ...
    runTest("Rule List-two rules, 1 head var, different in each-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(Y) :- r(Y)'));
    ...
    runTest("Rule List-two rules, 1 with head var, 1 with head constant-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(Y) :- r(Y)'));
    ...
    runTest("Rule List-two rules, each with different constant-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(d) :- r(Y)'));
    ...
    runTest("Rule List-two rules, each with same constant-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(c) :- r(Y)'));
    ...
    runTest("Rule List-two rules, 1 with head var, 1 with compound term-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(X) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(f(X,Y)) :- r(Y)'));
    ...
    runTest("Rule List-two rules, 1 with compound term, 1 with constant-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X,Y)) :- r(Y)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(c) :- r(Y)'));
    ...
    runTest("Rule List-two rules, complex-1-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c,X,d) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(X,Y,Y) :- r(X) & s(Y) & t(Y)'));
    ...
    runTest("Rule List-two rules, complex-2-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c,X,d) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(X,Y,f(Y)) :- r(X) & s(Y) & t(f(Y))'));
    ...
    runTest("Rule List-two rules, complex-3-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(c,X,d) :- q(X)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(X,Y,f(Y)) :- r(X) & s(Z) & t(W,4)'));
    ...
    runTest("Rule List-two rules, complex-4-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('ans(c)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('ans(W) :- q(X,W)'));
    ...
    runTest("Rule List-two rules, complex-5-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('ans(c, d)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('ans(W, f(Z)) :- q(X,W)'));
    ...
    runTest("Rule List-multiple rules, complex-success", () => {
        let rule1: Rule = EpilogJSToTS.parseRule(read('p(f(X), c)'));
        let rule2: Rule = EpilogJSToTS.parseRule(read('p(X, f(X))'));
        let rule3: Rule = EpilogJSToTS.parseRule(read('p(f(X), X)'));
        let rule4: Rule = EpilogJSToTS.parseRule(read('p(f(X,Y), X)'))
        let rule5: Rule = EpilogJSToTS.parseRule(read('p(Y, f(X,Y))'))

        let ruleAsFOL : Formula = EpilogTSToFOL.parseRuleList([rule1, rule2, rule3, rule4, rule5]);

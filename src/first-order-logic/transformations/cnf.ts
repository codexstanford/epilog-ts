import { Atom, ERROR_ATOM } from "../../epilog-ts-core/classes/Atom.js";
...
function toCNF_helper_Tseitins(formula : Formula, tseitinPropVarForFormula : Atom, tseitinVarCounter : number) : [Conjunction, number] {
    ...
    if (formula instanceof Conjunction) {
        ...
        let conjunctInCNFAccumulator : Formula[] = [];
        let tseitinVarConjuncts : Formula[] = [];
        let finalTseitinVarConjunctDisjuncts : Formula[] = [new Literal(tseitinPropVarForFormula, false)];
        ...
        for (let currConjunct of formula.conjuncts) {
            ...
            let newTseitinVar : Atom = new Atom(new Predicate(TSEITIN_VAR_PREFIX+tseitinVarCounter), []);
            tseitinVarCounter++;

            // Convert to CNF.
            let currConjunctInCNF : Conjunction = ERROR_CONJUNCTION;
        ...
        let cnfResult : Conjunction = new Conjunction([...tseitinVarConjuncts, new Disjunction(finalTseitinVarConjunctDisjuncts), ...conjunctInCNFAccumulator]);
    ...
    if (formula instanceof Disjunction) {
        ...
        let disjunctInCNFAccumulator : Formula[] = [];
        let tseitinVarConjuncts : Formula[] = [];
        let finalTseitinVarConjunctDisjuncts : Formula[] = [new Negation(new Literal(tseitinPropVarForFormula, false))];
        ...
        for (let currDisjunct of formula.disjuncts) {
            ...
            // Convert to CNF.
            let currDisjunctInCNF : Conjunction = ERROR_CONJUNCTION;
        ...
        let cnfResult : Conjunction = new Conjunction([...tseitinVarConjuncts, new Disjunction(finalTseitinVarConjunctDisjuncts), ...disjunctInCNFAccumulator]);
        return [cnfResult, tseitinVarCounter]
...
function toCNF_helper_standard(formula: Formula) : Conjunction {

    if (formula instanceof Conjunction) {
        let conjunctDisjunctions : Formula[] = [];
    ...
    if (formula instanceof Disjunction) {
        ...
        let distributedCNFDisjuncts : Formula[][] = [[]];

        for (let cnfFormula of cnfDisjuncts) {
            let tempCNFDisjuncts : Formula[][] = [];
...
function toCNF(initialFormula : Formula, options : CNFOptions = {algorithm: "tseitins"} ) : Conjunction {
    ...
    if (options.algorithm === "tseitins") {
        ...
        let topTseitinVar : Atom = new Atom(new Predicate(TSEITIN_VAR_PREFIX+"0"), []);
    
        let cnfFormula : Conjunction = toCNF_helper_Tseitins(initialFormula, topTseitinVar, 1)[0];

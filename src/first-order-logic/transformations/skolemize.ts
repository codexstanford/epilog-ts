...
import { CompoundTerm, Term, Variable } from "../../epilog-ts-core/classes/Term.js";
...
function skolemize_helper(formula : Formula, skolemFunctionCounter: number, skolemizingSub : Substitution, currentUnivVars : Variable[]) : [Formula, number]  {
    ...
    if (formula instanceof Negation) {
        let modifiedTarget : Formula = ERROR_FORMULA;
    ...
    if (formula instanceof Implication) {
        let modifiedAntecedent : Formula = ERROR_FORMULA;
        let modifiedConsequent : Formula = ERROR_FORMULA;
    ...
    if (formula instanceof Conjunction) {
        ...
        for (let conjunct of formula.conjuncts) {
            let currModifiedConjunct : Formula = ERROR_FORMULA;
    ...
    if (formula instanceof Disjunction) {
        ...
        for (let disjunct of formula.disjuncts) {
            let currModifiedDisjunct : Formula = ERROR_FORMULA;
    ...
    if (formula instanceof QuantifiedFormula) {
        if (formula.quantifier === Quantifier.Universal) {
            let modifiedFormula : Formula = ERROR_FORMULA;
        ...
        if (formula.quantifier === Quantifier.Existential) {
            let newSkolemTerm : CompoundTerm = new CompoundTerm(new Constructor("skolemfunc"+skolemFunctionCounter), currentUnivVars);
...
function skolemize(formula : Formula) : Formula {

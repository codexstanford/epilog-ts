...
function standardizeVarNames_helper(initialFormula, varCounter, sub) {
    if (initialFormula instanceof QuantifiedFormula) {
        let varName = initialFormula.variable.name;
        let newVar = new Variable("V" + varCounter);
        varCounter++;
        let subbedResult = ERROR_FORMULA;
    ...
    if (initialFormula instanceof Literal) {
        let updatedLiteral = Literal.applySub(sub, initialFormula);
        return [updatedLiteral, varCounter];
    }
    if (initialFormula instanceof Negation) {
        let updatedTarget = ERROR_FORMULA;
    ...
    if (initialFormula instanceof Conjunction) {
        ...
        for (let conjunct of initialFormula.conjuncts) {
            let updatedConjunct = ERROR_FORMULA;
    ...
    if (initialFormula instanceof Disjunction) {
        ...
        for (let disjunct of initialFormula.disjuncts) {
            let updatedDisjunct = ERROR_FORMULA;
    ...
    if (initialFormula instanceof Implication) {
        let updatedAntecedent = ERROR_FORMULA;
        let updatedConsequent = ERROR_FORMULA;

...
import { Symbol, Variable } from "../classes/Term.js";
...
function runUnifyTests() {
    ...
    let sym1 = new Symbol("c1");
    let sym2 = new Symbol("c2");
    let varX = new Variable("X");
    let varY = new Variable("Y");
    let varZ = new Variable("Z");
    let compoundTerm1 = EpilogJSToTS.parseCompoundTerm(read("f(c1)"));
    let compoundTerm2 = EpilogJSToTS.parseCompoundTerm(read("f(c2)"));
    let compoundTerm3 = EpilogJSToTS.parseCompoundTerm(read("f(X)"));
    let compoundTerm3_b = EpilogJSToTS.parseCompoundTerm(read("f(Y)"));
    let compoundTerm4 = EpilogJSToTS.parseCompoundTerm(read("g(f(X))"));
    let compoundTerm5 = EpilogJSToTS.parseCompoundTerm(read("g(c1)"));
    let compoundTerm6 = EpilogJSToTS.parseCompoundTerm(read("h(X, X)"));
    let compoundTerm7 = EpilogJSToTS.parseCompoundTerm(read("h(Y, Y)"));
    let compoundTerm8 = EpilogJSToTS.parseCompoundTerm(read("f(Y, Y)"));
    let compoundTerm9 = EpilogJSToTS.parseCompoundTerm(read("h(c1, c2)"));
    let compoundTerm10 = EpilogJSToTS.parseCompoundTerm(read("h(f(Z), Z)"));
    let compoundTerm11 = EpilogJSToTS.parseCompoundTerm(read("h(Z, f(Z))"));
    let compoundTerm12 = EpilogJSToTS.parseCompoundTerm(read("h(Y, f(Y))"));
    // Same constants
    ...
    let p_c1 = EpilogJSToTS.parseLiteral(read("p(c1)"));
    let p_c2 = EpilogJSToTS.parseLiteral(read("p(c2)"));
    let p_X = EpilogJSToTS.parseLiteral(read("p(X)"));
    let p_Y = EpilogJSToTS.parseLiteral(read("p(Y)"));
    let p_X_Y = EpilogJSToTS.parseLiteral(read("p(X, Y)"));
    let p_Z_W = EpilogJSToTS.parseLiteral(read("p(Z, W)"));
    let p_c1_c2 = EpilogJSToTS.parseLiteral(read("p(c1, c2)"));
    let neg_p_c1 = EpilogJSToTS.parseLiteral(read("~p(c1)"));
    let neg_p_c2 = EpilogJSToTS.parseLiteral(read("~p(c2)"));
    let neg_p_X = EpilogJSToTS.parseLiteral(read("~p(X)"));
    let neg_p_Y = EpilogJSToTS.parseLiteral(read("~p(Y)"));
    let p_X_fX = EpilogJSToTS.parseLiteral(read("p(X, f(X))"));
    let p_Y_Y = EpilogJSToTS.parseLiteral(read("p(Y, Y)"));
